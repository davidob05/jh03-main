...
SELECT is_unique('venue', 'venue_id');
...
-- Exam checks (2)
SELECT isnt_null('SELECT start_time FROM exam', 'Exam.start_time is NOT NULL');
SELECT isnt_null('SELECT course_code FROM exam', 'Exam.course_code is NOT NULL');

-- Business Logic (8)
SELECT ok((SELECT COUNT(*) FROM exam WHERE exam_length <0) = 0, 'Exam length should not be negative');
...
SELECT ok((SELECT COUNT(*) FROM examvenue WHERE EXTRACT(HOUR FROM adj_starttime)<0) = 0, 'Adjusted exam time should not be before 9am');
SELECT ok((SELECT COUNT(*)
                FROM studentexam AS se1
                JOIN exam e1 ON se1.exam_id = e1.exam_id
                JOIN studentexam se2 ON se1.student_id = se2.student_id
                JOIN exam e2 ON se2.exam_id = e2.exam_id
                WHERE se1.exam_id < se2.exam_id
                        AND e1.start_time < e2.start_time + INTERVAL '1 minute' * e2.exam_length
                        AND e2.start_time < e1.start_time + INTERVAL '1 minute' * e1.exam_length )=0, 'Student has an exam clash'); 
SELECT ok((SELECT COUNT(*)
                FROM provisions p
...
SELECT ok((SELECT COUNT(*) FROM exam
